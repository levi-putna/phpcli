...
use PHPCli\Cell\Cell;
use PHPCli\Cell\CellFormatInterface;
...
class Table
{
    ...
    public function __construct($itemName = 'Row', $useColors = true)
    {
        ...
        $this->tableColor = Color::$blue;
        $this->headerColor = Color::$light_blue;
    }
     ...
     * @return void
     */
    public function addField($fieldName, $fieldKey, $manipulator = false, $color = null)
    {

        if(is_null($color)){
           $color = \PHPCli\Color::$white;
        }
            ...
            if (count($this->injectedData)) {
                foreach ($this->injectedData as $row) {
                    ...
                    foreach ($this->fields as $field) {
                        ...
                        if ($field['manipulator'] instanceof CellFormatInterface) {
                            $cell = $field['manipulator'];
                        }else{
                            $cell = new Cell();
                        }

                        $value = $cell->format($value, $row, $field['name']);
    ...
    protected function getFormattedRow($rowData, $columnLengths, $header = false)
    {
        ...
        foreach ($rowData as $key => $field) {
            ...
            $field = ' ' . ($this->getUseColors() ? $this->getColor($color) : '') . $field;
            $response .= $field;
    ...
    protected function getChar($type, $length = 1)
    {
        ...
        if (isset($this->chars[$type])) {
            if ($this->getUseColors()) {
                $response .= $this->getColor($this->getTableColor());
            }
